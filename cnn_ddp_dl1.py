...
class CellClassification(ImageClassificationBase):
    def __init__(self):
        ...
        self.network = nn.Sequential(
            ...
            nn.Linear(1600,128),
            nn.BatchNorm1d(128, eps=0.001, momentum=0.99),
            nn.ReLU(),
            nn.Dropout(0.2),
...
def train_model(rank, args):
    ...
    dist.init_process_group(backend="hccl", rank=rank, world_size=args.world_size)
    ...
    args.batch_size = 32
    args.device = device = torch.device('hpu', rank)
    ...
    if is_first_local_rank:
        trainset = CustomDataset(root_dir=train_dir, dim=75, channels=channels, TotalSamples=10000)
    ...
    testset = CustomDataset(root_dir=test_dir, dim=75, channels=channels, TotalSamples=1000)
